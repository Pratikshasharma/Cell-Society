...
public class Segregation {
	...
	/*
	private boolean checkIsMyRace(Cell myCell, int row, int column){
		return myCell.getCellCurrentState().getStateID() == myGrid[row][column].getCellCurrentState().getStateID();
	}
	*/
	...
	private void findEmptyCell(int currentRow, int currentColumn){
		for(int i = 0; i<myGrid.length; i++){
			for (int j = 0; j<myGrid[i].length; j++){
				if((myGrid[i][j].getNextState() == null || myGrid[i][j].getNextState().getStateID() == 0)
	...
	public void updateSimulation(){
		for(int i = 0; i<myGrid.length; i++){
			for(int j = 0; j<myGrid[i].length; j++){
				...
				if(isSatisfied(myRaceCount/otherRaceCount)){
					keepState(i, j);
				}
				else{
					findEmptyCell(i, j);
